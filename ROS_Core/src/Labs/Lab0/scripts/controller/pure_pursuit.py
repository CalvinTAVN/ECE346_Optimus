...
class PurePursuitController():
    ...
    def goal_callback(self, goal_msg: PoseStamped):
        ...
        main_goal = np.array([goal_x, goal_y, 1])
        self.goal_buffer.writeFromNonRT(main_goal)
        ########################### END OF TODO 3 #################################
    ...
    def publish_control(self, accel, steer, state):
        ...
        new_servo_msg = ServoMsg()
        new_servo_msg.header.stamp = rospy.Time.now()
        new_servo_msg.throttle = throttle
        new_servo_msg.steer = steer
        self.control_pub.publish(new_servo_msg)
